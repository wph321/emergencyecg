...
public class MainActivityqny extends AppCompatActivity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if (mUserName.equals("")) {
            ...
            Log.e("TAG", "roomName: "+mRoomName+"----------mUserName"+mUserName );
...
//            checkUpdate();
    ...
    private void startConference(final String roomName) {
        ...
        new Thread(new Runnable() {
            @Override
            public void run() {
                final String token = QNAppServer.getInstance().requestRoomToken(MainActivityqny.this, mUserName, roomName);

                runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        if (token == null) {
                            ToastUtils.s(MainActivityqny.this, getString(R.string.null_room_token_toast));
                            return;
                        }
                        Intent intent = new Intent(MainActivityqny.this, mIsScreenCaptureEnabled ? ScreenCaptureActivity.class : RoomActivity.class);
    ...
    public void onClickLiveRoom(View v) {
        ...
        new Thread(new Runnable() {
            @Override
            public void run() {
                UserList userList = QNAppServer.getInstance().getUserList(MainActivityqny.this, mRoomName);
                ...
                final String token = QNAppServer.getInstance().requestRoomToken(MainActivityqny.this, mUserName, mRoomName);
                runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        if (token == null) {
                        ...
                        Intent intent = new Intent(MainActivityqny.this, LiveRoomActivity.class);
    ...
    private boolean handleRoomInfo() {
        ...
        if (!MainActivityqny.isRoomNameOk(roomName)) {
    ...
    private void createProgressDialog() {
        mProgressDialog = new ProgressDialog(MainActivityqny.this);
    ...
    private void goToDownload(String downloadUrl) {
        Intent intent = new Intent(MainActivityqny.this, DownloadService.class);
